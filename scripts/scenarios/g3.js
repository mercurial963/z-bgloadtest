...
export function g3Flow(token, config) {
  ...
  group('G3', function () {
    ...
    if (!apId) {
      ...
      sleep(1);
      getStep(2, `/account-profile/${apId}`, 'account-profile/{id} (detail)');
      // 3. account-profile edit history (id chained from step 1)
      sleep(1);
      getStep(3, `/account-profile/${apId}/history`, 'account-profile/{id}/history');
    }

    // 4. control-account (บัญชีคุม) list — independent read.
    sleep(1);
    getStep(4, '/account-profile/parents', 'account-profile/parents');
    ...
    if (!accountItemId) {
      ...
      sleep(1);
      getStep(6, `/account-item/${accountItemId}`, 'account-item/{id} (detail)');
      // 7. account-item edit history (id from step 5)
      sleep(1);
      getStep(7, `/account-item/${accountItemId}/history`, 'account-item/{id}/history');
      // 8. account-item + contra pairing by สปส. branch 1000 (id from step 5)
      sleep(1);
      getStep(8, `/account-item/${accountItemId}/1000`, 'account-item/{id}/{ssoCode}');
    }
    ...
    sleep(1);
    getStep(11, '/remain-balance/passbook-history/24367009', 'remain-balance/passbook-history/{id}');
    ...
    const tc = getStep(
      12,
      '/sso-bank-transfer-central?jobCode=&createdDate=&status=&page=0&size=20&sort=ASC',
    ...
    const il = getStep(
      14,
      '/item-ledger?ssoBranchCode=1000&transactionDateFrom=&transactionDateTo=&accountLedgerType=&accountDocumentCode=&itemLedgerStatus=&accountFiPeriod=&accountFiYear=2569&createdDate=&page=0&size=20&sort=ASC',
      'item-ledger (search)'
    );
    ...
    sleep(1);
    getStep(15, `/item-ledger/${accountLedgerId}`, 'item-ledger/{id} (detail)');

    // === account-closed (ปิดบัญชี) ====================================
    sleep(1);
    getStep(16, '/account-closed/sso-yearly?fiYear=2567&ssoBranchCode=1000', 'account-closed/sso-yearly');
    sleep(1);
    getStep(17, '/account-closed/sso-monthly?fiYear=2567&monthDigit=01', 'account-closed/sso-monthly');
    sleep(1);
    getStep(
      18,
      '/account-closed/sso-monthly-history?fiYear=2567&monthDigit=01&ssoBranchCode=1000',
      'account-closed/sso-monthly-history'
    );
    sleep(1);
    getStep(19, '/account-closed?fiYear=2567&ssoBranchCode=1000', 'account-closed (info)');

    // === ledger inquiry + masterdata ==================================
    sleep(1);
    getStep(
      20,
      '/inquiry-general-ledger?accountProfileId=11210600&fiYear=2567&month=JANUARY&ssoCode=1000&reportFlag=EXCEL&download=false',
      'inquiry-general-ledger'
    );
    sleep(1);
    getStep(21, '/account-type', 'account-type');
    ...
    sleep(1);
    postStep(22, '/report/acc5059', { ssoCode: '1000', reportFlag: 'PDF', download: true });
    sleep(1);
    postStep(23, '/report/acc5032', {
    ...
    sleep(1);
    postStep(27, '/report/acc5029', { accountLedgerId: 9463903, reportFlag: 'PDF', download: true });
    sleep(1);
    postStep(28, '/report/acc5028', {
